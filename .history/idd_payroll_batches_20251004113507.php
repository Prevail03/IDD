...
if($action == 2){
    ...
    $sql = "UPDATE MYDB.DBO.idd_payroll_batches SET status = 'Approved', approved_by = ?  approved_at = ? WHERE batch_id = ? AND scheme_code = ?";
    $params = array($_SESSION['username'], $approved_at, $batch_id, $_SESSION['scheme_code']);
    ...
    //3. Add to contributions mydb.dbo.contributions_flat_entries_tb $ contmydb.dbo.contributions_flat_batches_tb, batches_tb
    ...
    $sql_entries = "INSERT INTO mydb.dbo.contributions_flat_entries_tb
        ...
        SELECT  
            ...
            0.00 AS cont_flat_ee_nte,
            0.00 AS cont_flat_avc_te,
            0.00 AS cont_flat_avc_nte,
            0.00 AS cont_flat_er_te,
            0.00 AS cont_flat_er_nte,
            'Normal' AS cont_flat_type,
            CONVERT(date, GETDATE()) AS cont_flat_display_date,
            CONVERT(date, stg.created_at) AS cont_flat_date_paid,
            0.00 AS cont_flat_er_te_nssf,
            0.00 AS cont_flat_er_nte_nssf,
            0.00 AS cont_flat_ee_te_nssf,
            0.00 AS cont_flat_ee_nte_nssf
        FROM mydb.dbo.idd_payroll_batches idd
						  ...
						  <div class="dropdown-menu">
                            ...
                            <?php if($row['status'] === 'Open'){ ?>
                                <a class="dropdown-item" 
                                   href="./idd_payroll_batches.php?action=2&batch_id=<?php echo($row['batch_id']); ?>" 
                                   onclick="return confirm('Are you sure you want to Approve? This action is irreversible.');">
                                   Approve
                                </a>
                                <a class="dropdown-item" 
                                   href="./idd_payroll_batches.php?action=3&batch_id=<?php echo($row['batch_id']); ?>" 
                                   onclick="return confirm('Are you sure you want to Reject? This action is irreversible.');">
                                   Reject
                                </a>
                            <?php } else { ?>
                                <a class="dropdown-item disabled" href="#">Approve </a>
                                <a class="dropdown-item disabled" href="#">Reject</a>
                            <?php } ?>
