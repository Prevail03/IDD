...
if($action == 2){
    ...
    $sql = "UPDATE MYDB.DBO.idd_payroll_batches SET status = 'Approved', approved_by WHERE batch_id = ? AND scheme_code = ?";
    ...
    $sql = "UPDATE MYDB.DBO.idd_payroll_staging SET status = 'Approved' WHERE batch_id = ? AND scheme_code = ?";
    $params = array($batch_id,$_SESSION['scheme_code']);
    $stmt = sqlsrv_query( $conn, $sql ,$params);  
    if( $stmt === false) {
        echo "Error in payroll statements statement execution.\n";
      die( print_r( sqlsrv_errors(), true) );
    } 
    ...
    $_SESSION['add_message'] = "Payroll batch approved successfully";
					...
					<td>
						<div class="dropdown">
						  ...
						  <div class="dropdown-menu">
						    ...
						    <a class="dropdown-item" href="./pen_payrolls_processed.php?action=2&batch_id=<?php echo($row['batch_id']); ?>">Approve </a>
