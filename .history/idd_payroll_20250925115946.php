        ...
        <?php
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
                    ...
                    // 2. Main payroll query with medical, exceptions, and advances
                    $sql = "
                    SELECT 
                        p.insert_id,
                        p.member_number,
                    ...
                    while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                        ...
                        if (!empty($row['payment_frequency'])) {
                            ...
                            if ($row['payment_frequency'] == 'Annually' && $month != 12) {
                                $include = false;
                            }
                            if ($row['payment_frequency'] == 'Semi-Annually' && !in_array($month, [6, 12])) {
                                $include = false;
                            }
                        ...
                        // 2. Advance filter: exclude if selected sub-period falls inside advance range
