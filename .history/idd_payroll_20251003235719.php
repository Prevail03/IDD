...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
        ...
        // 2. Create a new batch for this run
        $sql_batch = "INSERT INTO mydb.dbo.idd_payroll_batches (scheme_code, period_id, sub_period_id, payroll_date, created_at)
                      OUTPUT INSERTED.batch_id
                      VALUES (?, ?, ?, ?, GETDATE())";
        $params_batch = [$scheme_code, $period, $selected_end, $sub_period];
        $stmt_batch = sqlsrv_query($conn, $sql_batch, $params_batch);
        if ($stmt_batch === false) {
            die("Error creating batch: " . print_r(sqlsrv_errors(), true));
        }
        $batch_row = sqlsrv_fetch_array($stmt_batch, SQLSRV_FETCH_ASSOC);
        $batch_id  = $batch_row['batch_id'];

        // 3. Main payroll query
        $sql = "
        SELECT 
            p.insert_id,
            p.member_number,
        ...
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            // Exclude if advance period covers selected sub-period
            if (!empty($row['adv_start_date']) && !empty($row['adv_end_date'])) {
                if ($selected_start >= $row['adv_start_date'] && $selected_end <= $row['adv_end_date']) {
                    $include = false;
                }
            }

            // Expired check: >10 years since drawdown
            if (!empty($row['drawdown_commence_date'])) {
                $ten_years = (clone $row['drawdown_commence_date'])->modify('+10 years');
                if ($selected_end >= $ten_years) {
                    $include = false;
                    $expired_rows[] = $row;
                }
            }
            ...
            if (!empty($row['payment_frequency'])) {
                switch ($row['payment_frequency']) {
                    case 'Annually':
                        if ($month == 12) $scale_factor = 12;
                        else $include = false;
                        break;
                    case 'Semi-Annually':
                        if (in_array($month, [6, 12])) $scale_factor = 6;
                        else $include = false;
                        break;
                    case 'Quarterly':
                        if (in_array($month, [3, 6, 9, 12])) $scale_factor = 3;
                        else $include = false;
                        break;
                    default:
                        $scale_factor = 1;
            ...
            if ($include) {
                $valid_rows[] = [
                    ...
                    'drawdown_pct'  => $row['absolute_drawdown_percentage']
                ];

                $sql_stage = "INSERT INTO mydb.dbo.idd_payroll_staging 
                              (scheme_code, member_number, member_name, gross_salary, medical_deduction, net_salary, payroll_date, closing_balance, drawdown_percentage, status, created_at, batch_id)
                ...
                $params_stage = [
                    ...
                    $selected_end,
                    $row['closing_balance'],
                    $row['absolute_drawdown_percentage'],
                    $batch_id
                ];
                $stmt_stage = sqlsrv_query($conn, $sql_stage, $params_stage);
                if ($stmt_stage === false) {
                    die("Error inserting staging row: " . print_r(sqlsrv_errors(), true));
                }
            } elseif ($overpaid) {
                $overpaid_rows[] = $row;
            } else {
        ...
        // === Output payroll preview ===
        echo "<h4>Payroll Disbursement List (Staged)</h4>";
        ...
        echo "<table class='table table-bordered table-striped'><thead><tr>
                ...
                <th>Gross</th><th>Medical</th><th>Net Pay</th>
            </tr></thead><tbody>";
        $i=1;
