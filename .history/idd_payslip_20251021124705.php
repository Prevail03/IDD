...
$sql = "DECLARE @PrevYear INT = YEAR(GETDATE()) - 1;
SELECT  
    ist.staging_id,
    ist.scheme_code,
    ist.member_number,
    ...
    s.sub_period_name,
    m.m_email,
    ISNULL(bal.balance_at_end_year, 0) AS balance_at_end_year
...
LEFT JOIN (
    SELECT  
        cont_scheme_code,  
        cont_member_number,  
        SUM(cont_amount) AS balance_at_end_year
    FROM mydb.dbo.contributions_tb
    WHERE cont_display_date BETWEEN 
          DATEFROMPARTS(2016, 1, 1)  -- or earliest system date
          AND DATEFROMPARTS(@PrevYear, 12, 31)
    GROUP BY cont_scheme_code, cont_member_number
) AS bal
    ON bal.cont_scheme_code = ist.scheme_code
   AND bal.cont_member_number = ist.member_number

WHERE LTRIM(RTRIM(ist.status)) = 'Approved'
  AND ist.batch_id = 8
ORDER BY ist.member_name;
";

$stmtOptions = ["Scrollable" => SQLSRV_CURSOR_FORWARD, "QueryTimeout" => 600];
$result = sqlsrv_query($conn, $sql, [], $stmtOptions);
...
while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
    ...
    $balanceAtEndYear = number_format($row['balance_at_end_year'], 2);
    ...
    $filename = $payslipPath . 'payslip_' . preg_replace('/[^A-Za-z0-9_\-]/', '_', $memberName) . '_' . $timestamp . '.pdf';
