    ...
    if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
             ...
             WHERE scheme_code = ? AND member_number = ?";
                ...
                if ($exists['cnt'] == 0) {
                    ...
                    $sql = "INSERT INTO MYDB.DBO.idd_payroll 
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $params = [$scheme_code, $member_number, $member_name, $account_number, $bank_code, $gross_after, $added_by];
        ...
        if ($exists['cnt'] == 0) {
            $sql = "INSERT INTO MYDB.DBO.idd_medical 
                    (payroll_id, medical_amount, added_by) 
                    VALUES (?, ?, ?)";
            $params = [$payroll_id, $amount, $added_by];
            $stmt = sqlsrv_query($conn, $sql, $params);
        ...
        $sql = "INSERT INTO MYDB.DBO.idd_payroll_exceptions
        ...
        <div class="row"  x-data="initiatePayrollForm()" >
            ...
            <div class="col-sm-12">
                <div class="d-flex flex-wrap">
                    <button type="button" class="btn btn-primary mr-2" data-toggle="modal" data-target="#addPayrollModal">Add Payroll Record</button>
                    <button type="button" class="btn btn-primary mr-2" data-toggle="modal" data-target="#addMedicalModal">Medical</button>
                    <button type="button" class="btn btn-primary mr-2" data-toggle="modal" data-target="#applyAdvanceModal">Apply for Advance</button>
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addExceptionModal">Exceptions</button>
                </div>
            </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <div class="form-group ">
                ...
                <select id="payrollSelect" class="form-control" name="payroll_id">
                    <?php
                    $scheme_code = $_SESSION['scheme_code'];
                    $sql = "SELECT * FROM idd_payroll WHERE scheme_code = '$scheme_code' ORDER BY member_name ASC";
                    $stmt = sqlsrv_query($conn, $sql, $params);
                    if ($stmt === false) {
                        throw new Exception('Failed to get member details from DB. ERROR: '.print_r(sqlsrv_errors(), true));
                    }

                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                        echo '<option value="'.$row['insert_id'].'">'.$row['member_name'].' ('.$row['member_number'].')</option>';
                    }
                    ?>
                </select>
...
<!-- Apply for advance -->
<div class="modal fade" id="applyAdvanceModal" tabindex="-1" role="dialog" aria-labelledby="addPayrollLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div>
        <div class="modal-content">
            <form action="load_payroll.php" method="post">
                <div class="modal-header">
                <h5 class="modal-title" id="addPayrollLabel">Apply for Advance</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                </div>
                ...
                <div class="modal-body">
                    <input type="hidden" name="form_name" value="apply_advance"/>
                    <input type="hidden" name="scheme_code" value="<?php echo($_SESSION['scheme_code']); ?>"/>
                    &nbsp;&nbsp;
                    <div class="form-group">
                        <div class="form-group ">
                            <label for="sel1">Select Member:</label>
                            <select id="advanceSelect" class="form-control" name="payroll_id">
                                <?php
                                $scheme_code = $_SESSION['scheme_code'];
                                $sql = "SELECT * FROM idd_payroll WHERE scheme_code = '$scheme_code' ORDER BY member_name ASC";
                                $stmt = sqlsrv_query($conn, $sql, $params);
                    ...
                    <label>Advance Period Start</label>
                    <div class="form-group row">
                        <div class="col">
                            <label class="form-label">Select Period:</label>
                            <select class="form-control" id="startPeriodSelect" name= "start_period" >
                        ...
                        <div class="col">
                            ...
                            <select class="form-control" id="startSubPeriodSelect" name= "start_sub_period" >
                    ...
                    <!-- Advance Period End -->
                    <label>Advance Period End</label>
                    <div class="form-group row">
                        <div class="col">
                            <label class="form-label">Select Period:</label>
                            <select class="form-control" id="endPeriodSelect" name= "end_period" >
                                <option value="">-- Select Period --</option>
                                <?php
                                // Same query as above
                                $sql = "SELECT * FROM scheme_periods_tb 
                        ...
                        <div class="col">
                            <label class="form-label">Select Sub Period</label>
                            <select class="form-control" id="endSubPeriodSelect" name= "end_sub_period" >
                                <option value="">-- Select Sub Period --</option>
                            </select>
                        </div>
                    </div>
...
<!-- Exceptions Modal -->
<div class="modal fade" id="addExceptionModal" tabindex="-1" role="dialog" aria-labelledby="addPayrollLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <form action="load_payroll.php" method="post">
        <div class="modal-header">
          <h5 class="modal-title" id="addPayrollLabel">Add Payroll Exceptions</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        ...
        <div class="modal-body">
          <input type="hidden" name="form_name" value="add_exception"/>
          <input type="hidden" name="scheme_code" value="<?php echo($_SESSION['scheme_code']); ?>"/>

          <div class="form-group">
            <label for="exceptionSelect">Select Member:</label>
            <select id="exceptionSelect" class="form-control" name="payroll_id">
              <?php
              $scheme_code = $_SESSION['scheme_code'];
              $sql = "SELECT * FROM idd_payroll WHERE scheme_code = '$scheme_code' ORDER BY member_name ASC";
              $stmt = sqlsrv_query($conn, $sql, $params);
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    const element = document.getElementById('payrollSelect');
    const choices = new Choices(element, {
      searchEnabled: true,
      itemSelectText: '',   // removes "Press to select" text
      shouldSort: false     // keeps your SQL order
    });
    const advance_element = document.getElementById('advanceSelect');
    const advance_choices = new Choices(advance_element, {
      searchEnabled: true,
      itemSelectText: '',   // removes "Press to select" text
      shouldSort: false     // keeps your SQL order
    });
    const exception_element = document.getElementById('exceptionSelect');
    const exception_choices = new Choices(exception_element, {
      searchEnabled: true,
      itemSelectText: '',   // removes "Press to select" text
      shouldSort: false     // keeps your SQL order
    });
...
document.getElementById('startPeriodSelect')
    .addEventListener('change', () => loadSubPeriods('startPeriodSelect', 'startSubPeriodSelect'));

document.getElementById('endPeriodSelect')
    .addEventListener('change', () => loadSubPeriods('endPeriodSelect', 'endSubPeriodSelect'));
</script>
