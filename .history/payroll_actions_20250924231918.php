    ...
    <div class="container-fluid  px-5 py-3">
        ...
        <div class="row">
            ...
            <div class="tab-pane fade show active p-3" id="payroll" role="tabpanel" aria-labelledby="payroll-tab">
                ...
                <?php
                    $sql = "SELECT * FROM MYDB.DBO.idd_payroll WHERE scheme_code = '$scheme_code' ORDER BY insert_id DESC";
                            ...
                            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) { ?>
                                <tr>
                                    <form action="idd_payroll_actions.php" method="POST">
                                        <td><?php echo $i++; ?></td>
                                        <td>
                                            <input type="text" name="member_number" class="form-control form-control-sm" 
                                                value="<?php echo htmlspecialchars($row['member_number']); ?>">
                                        </td>
                                        <td>
                                            <input type="text" name="member_name" class="form-control form-control-sm" 
                                                value="<?php echo htmlspecialchars($row['member_name']); ?>">
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="number" name="bank_code" class="form-control form-control-sm" 
                                                value="<?php echo htmlspecialchars($row['bank_code']); ?>">
                                        </td>
                                        <td>
                                            <input type="number" step="0.01" name="gross_after_deductions" class="form-control form-control-sm" 
                                                value="<?php echo htmlspecialchars($row['gross_after_deductions']); ?>">
                                        </td>
                                        <td><?php echo $row['created_at']->format('Y-m-d H:i:s'); ?></td>
                                        <td>
                                            <input type="hidden" name="form_name" value="updateuser">
                                            <input type="hidden" name="user_id" value="<?php echo $row['insert_id']; ?>">
                                            <button type="submit" class="btn btn-sm btn-primary">Save</button>
                                            <a href="idd_payroll_actions.php?delete=<?php echo $row['insert_id']; ?>" 
