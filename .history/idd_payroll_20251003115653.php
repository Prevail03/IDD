        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
                ...
                while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                    ...
                    $net     = $gross - $medical;
                    $include = true;
                    // === 1. Payment frequency filter ===
                    // === 1. Exceptions filter & scaling ===
                    if (!empty($row['payment_frequency'])) {
                        $month = (int)$selected_end->format('n');

                        if ($row['payment_frequency'] == 'Annually') {
                            if ($month == 12) {
                                $net = $net * 12;   // Pay 12 months in December
                                $gross = $gross * 12;
                                $medical = $medical * 12;
                            } else {
                                $include = false;
                        ...
                        elseif ($row['payment_frequency'] == 'Semi-Annually') {
                            if (in_array($month, [6, 12])) {
                                $net = $net * 6;    // Pay 6 months in June & December
                                $gross = $gross * 6;
                                $medical = $medical * 6;
                            } else {
                                $include = false;
                        ...
                        elseif ($row['payment_frequency'] == 'Quarterly') {
                            if (in_array($month, [3, 6, 9, 12])) {
                                $net = $net * 3;    // Pay 3 months in March, June, Sept, Dec
                                $gross = $gross * 3;
                                $medical = $medical * 3;
                            } else {
                                $include = false;
                    ...
                    if (!empty($row['advance_id'])) {
                        $adv_start = $row['adv_start_date'];
                        $adv_end   = $row['adv_end_date'];
                    ...
                    if (!empty($row['drawdown_commence_date'])) {
                        $cutoff = clone $row['drawdown_commence_date'];
                        $cutoff->modify('+10 years');
                        if ($selected_end > $cutoff) {
                            $expired = true;
                            $include = false;
                        }
                    }
                    ...
                    // === 4. Overpayment check ===
                    $overpaid = false;
                    $allowed_annual = ($row['absolute_drawdown_percentage'] * $row['closing_balance']) / 100;
                    ...
                    // Bucket results
                    if ($expired) {
                        $expired_rows[] = $row + ['gross'=>$gross,'medical'=>$medical,'net'=>$net];
                    } elseif ($overpaid) {
                        $overpaid_rows[] = $row + ['gross'=>$gross,'medical'=>$medical,'net'=>$net,'withdrawals'=>$withdrawals,'allowed'=>$allowed_annual];
                    } elseif ($include) {
                        $valid_rows[] = $row + ['gross'=>$gross,'medical'=>$medical,'net'=>$net];
                    }
                }
