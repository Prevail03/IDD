...
if($action == 2){
    ...
    $sql = "INSERT INTO [MYDB].[dbo].[batches_tb] (batch_description,batch_scheme_code,batch_gen_date,batch_auto_info) OUTPUT INSERTED.batch_id VALUES (?,?,?,?)";
    $params = array($flat_document, $flat_scheme_code, date('Y-m-d'), $batch_auto_info);
    $stmt_batch = sqlsrv_query( $conn, $sql ,$params );   
    if( $stmt_batch === false) {
        die( print_r( sqlsrv_errors(), true) );
    }
    ...
    $sql = "INSERT INTO [MYDB].[dbo].[contributions_flat_batches_tb] ( flat_scheme_code ,flat_batch_id ,flat_document ,flat_document_type ,flat_posting_date ,flat_initiated_by ,flat_initiated_on ,flat_approved_by ,flat_approved_on ,flat_approved ,flat_action_name ,flat_action_comment ) 
    VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?)";
    					
    $params = array($flat_scheme_code ,$flat_batch_id ,$flat_document ,$flat_document_type ,$flat_posting_date ,$flat_initiated_by ,$flat_initiated_on ,$flat_approved_by ,$flat_approved_on,$flat_approved,$flat_action_name,$flat_action_comments);
    ...
    $sql = "
        INSERT INTO mydb.dbo.contributions_flat_entries_tb
        (
            cont_scheme_code,
            cont_combined,
            cont_member_number,
    ...
    $sql = "
        INSERT INTO mydb.dbo.contributions_tb_testing
        (
            cont_scheme_code,
            cont_combined,
            cont_member_number,
