    ...
    if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
        ...
        while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            if (!empty($row['payment_frequency'])) {
                ...
                if ($row['payment_frequency'] == 'Annually' && $month != 12) $include = false;
                if ($row['payment_frequency'] == 'Semi-Annually' && !in_array($month, [6,12])) $include = false;
                if ($row['payment_frequency'] == 'Quarterly' && !in_array($month, [3,6,9,12])) $include = false;
            }
