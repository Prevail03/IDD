...
<div class="container-fluid  px-5 py-3">
	   ...
	   <div class="col-sm-12 mt-3 px-3 py-3">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_POST['form_name']) && $_POST['form_name'] === "load_payroll") {
                ...
                // 2. Create a new batch BEFORE looping
                $sql_batch = "INSERT INTO mydb.dbo.idd_payroll_batches 
                            (scheme_code, period_id, sub_period_id, payroll_date, status)
                            OUTPUT INSERTED.batch_id
                            VALUES (?, ?, ?, ?, 'Open')";
                $params_batch = [$scheme_code, $period, $sub_period, $selected_end];
                $stmt_batch = sqlsrv_query($conn, $sql_batch, $params_batch);
                if ($stmt_batch === false) {
                    die("Error creating payroll batch: " . print_r(sqlsrv_errors(), true));
                }
                ...
                while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                    ...
                    $member_number = $row['cont_member_number'];
    $member_name   = $row['m_name'];
    $closing_balance = $row['closing_balance'];
    $annual_withdrawal = $row['annual_withdrawal'];
    $net_salary = round($annual_withdrawal / 12, 2);
    $drawdown_percentage = round(($annual_withdrawal / NULLIF($closing_balance,0)) * 100, 2);

    // 4. Insert into STAGING inside the loop
    $sql_stage = "INSERT INTO mydb.dbo.idd_payroll_staging
        (batch_id, scheme_code, member_number, member_name, gross_salary, medical_deduction, net_salary, closing_balance, drawdown_percentage, payroll_date, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending')";
    ...
    $params_stage = [
        ...
        $member_number,
        $member_name,
        $annual_withdrawal, // gross salary = annual withdrawal
        0,                  // medical_deduction placeholder, update if you calculate it
        $net_salary,
        $closing_balance,
        $drawdown_percentage,
        $selected_end
    ...
    if ($stmt_stage === false) {
        die("Error inserting staging row: " . print_r(sqlsrv_errors(), true));
                ...
                echo "<table class='table table-bordered table-striped'><thead><tr>
                        ...
                        <th>Account</th><th>Bank</th><th>Gross</th>
                        <th>Medical</th><th>Net Pay</th>
                    </tr></thead><tbody>";
                $i=1;
