...
<body>
               ...
               <?php
                    $sql = "SELECT 
                        ...
                        INNER JOIN 
                            MYDB.DBO.sys_users_tb AS su 
                            ON su.user_id = idd.added_by
                            AND idd.insert_id = iddm.payroll_id
                            
                        WHERE 
                            iddm.scheme_code = '$scheme_code' and iddm.hidden_status= 'No'
                        ORDER BY 
                            idd.member_name ASC;
                        ";
                    $stmt = sqlsrv_query($conn, $sql);
                    ...
                    ?>
                        <!-- Search input -->
                        <input type="text" id="medicalSearch" class="form-control mb-3" placeholder="Search payroll records...">
                            ...
                            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) { ?>
                                <tr>
                                    <form action="idd_payroll_actions.php" method="POST">
                                        ...
                                        <td><?php echo htmlspecialchars($row['member_number']); ?></td>
                                        <td><?php echo htmlspecialchars($row['member_name']); ?></td>
                                        <td><input type="text" name="account_number" class="form-control form-control-sm" value="<?php echo htmlspecialchars($row['medical_amount']); ?>"></td>
                                        <td><?php echo htmlspecialchars($row['added_by']); ?></td>
                                        <td>
    ...
    <script>
        ...
        document.getElementById("medicalSearch").addEventListener("keyup", function() {
            let filter = this.value.toLowerCase();
            let rows = document.querySelectorAll("#medicalTable tbody tr");
