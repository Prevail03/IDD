...
if($action == 2){//Restore
	...
	$sql = "UPDATE payroll_tb SET payroll_status = ? WHERE payroll_id = ? AND payroll_scheme_code LIKE ?";
	$params = array(0,$payroll_id,$_SESSION['scheme_code']);
	$stmt = sqlsrv_query( $conn, $sql ,$params);
    if( $stmt === false) {
	  die( print_r( sqlsrv_errors(), true) );
	}
	
	$_SESSION['add_message'] = "Payroll restored successfully";
	
	header("location: ./pen_payrolls_processed.php");
	exit();
}
if($action == 3){//Check
	
	$sql = "UPDATE payroll_tb SET payroll_status = ?, payroll_checked_by = ? WHERE payroll_id = ? AND payroll_scheme_code LIKE ?";
	$params = array(2,$_SESSION['user_full_names'],$payroll_id,$_SESSION['scheme_code']);
	$stmt = sqlsrv_query( $conn, $sql ,$params);
    if( $stmt === false) {
	  die( print_r( sqlsrv_errors(), true) );
	}
	
	$_SESSION['add_message'] = "Payroll checked successfully";
...
<div class="container-fluid sticky-top" style="background-color: #eee;">
	...
	<div class="row">
		...
		<div class="col-sm-8">
			...
			<h3>DC Plan: Processed Payrolls</h3>
		</div>
	</div>
...
<div class="container-fluid">
	...
	<h2>Processed Pensioners Payrolls</h2>
