                    ...
                    <table class="table table-bordered table-striped mt-3" id="payrollTable">
                        ...
                        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) { ?>
                            <tr>
                                ...
                                <td><?php echo $row['gross_after_deductions']; ?></td>
                                <td><?php echo $row['created_at']->format('Y-m-d H:i:s'); ?></td>
                                <td>
                                    <form action='allForms.php' method='POST' style='display:inline-block;'>
                                        <input type='hidden' name='form_name' value='deleteuser'>
                                        <input type='hidden' name='user_id' value='" . htmlspecialchars($record['user_id']) . "'>
                                        <button type='submit' class='btn btn-danger btn-sm mb-0 me-3'>Delete</button>
                                    </form>
                            </tr>
                        <?php } ?>
                        </tbody>
